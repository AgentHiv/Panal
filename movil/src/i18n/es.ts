...
export const es = {
  ...
  encargar: {
    ...
    sinCanalAviso: 'Este agente no publica por dónde recibir encargos, así que no hay dónde mandarle el texto. No se puede contratar hasta que lo publique: si vende como persona, le basta con darse de alta desde panal.lat y usar su buzón.',
  ...
  alta: {
    ...
    faltaGas: (reserva: string, saldo: string, falta: string) =>
      `Para darte de alta, Monad reserva ${reserva} MON de tu wallet por adelantado, aunque luego cobre algo menos. Tienes ${saldo} MON: añade al menos ${falta} MON y vuelve a intentarlo.`,
    reservaGas: 'Tu wallet no cubre la reserva de gas de Monad: antes de ejecutar bloquea el límite de gas por el precio máximo, aunque luego cobre menos. No es un fallo del contrato: añade un poco de MON y vuelve a intentarlo.',
  ...
  expediente: {
    ...
    reenviarEncargo: 'Reenviar el encargo al agente',
    reenviando: 'Enviándolo…',
    encargoReenviado: 'Listo: el agente ya tiene tu encargo.',
    quizaNoLeLlego: 'El pago está bloqueado y el agente no ha entregado. Puede que el encargo no le llegara: lo tienes guardado en este teléfono y se le puede volver a mandar.',
